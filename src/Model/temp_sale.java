...
public class temp_sale {
	...
	public void snd_to_db_sale(){
		Connection is=Database.getConnection();
		
		String command="INSERT INTO `temp_sale`(`Product_id`, `Quantity`, `Buy_price`, `Total_Price`, `Sell_price`) VALUES (?,?,?,?,?)";
	...
	public static void load(){
		...
		String command="SELECT a.Name as Product,b.Quantity,b.Total_Price,b.Sell_price FROM products a,temp_sale b WHERE a.id=b.Product_id";
		try{
			PreparedStatement ps=is.prepareStatement(command);
			ResultSet rs=ps.executeQuery();
			
			Create_Sale.table.setModel(DbUtils.resultSetToTableModel(rs));
		}
		catch(Exception x){
			x.printStackTrace();
		}
	...
	public void prepare_update_sale(int id_n, int pd_id2, int quan, int bp, int sp, int total2){
		this.id=id_n;
		//this.dat=dat2;
		//this.vnd=vendor;
	...
	public void snd_to_db_update_sale(){
		Connection is=Database.getConnection();
		...
		String command="UPDATE `temp_sale` SET ``Product_id`=?,`Quantity`=?,`Buy_price`=?,`Total_Price`=?,`Sell_price`=? WHERE id=?";
		try{
			PreparedStatement ps=is.prepareStatement(command);
	...
	public void delete_temp(int id2){
		...
		String command="DELETE FROM `temp_sale` WHERE id=?";
	...
	public static int get_id_by_pd_id(int PD_id){
		...
		String command="SELECT id FROM `temp_sale` WHERE Product_id=?";
		int id=0;
		try{
			PreparedStatement ps=is.prepareStatement(command);
			ps.setInt(1,PD_id);
	...
	public static void truncate_temp_sale(){
		Connection is=Database.getConnection();
		
		String command="TRUNCATE TABLE `temp_sale`";
	...
	public static int get_total(){
		...
		String comnd="SELECT SUM(Total_Price) FROM temp_sale";
	...
	public static int ck_existing_pdt(int product_id){
		...
		String comnd="SELECT Product_id FROM `temp_sale` WHERE Product_id=?";
